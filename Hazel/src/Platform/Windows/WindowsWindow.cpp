...
namespace Hazel {
	...
	void WindowsWindow::Init(const WindowProps& props)
	{
		...
		if (!s_GLFWInitialized)
		{
			...
			HZ_CORE_ASSERT(success, "Could not intialize GLFW!"); // 仅在Debug版本中出现，因为它可能会终止窗口的创建，但这在Release版本中是不希望出现的
	...
	void WindowsWindow::SetVSync(bool enabled)
	{
		if (enabled) // 启用垂直同步
			...
			glfwSwapInterval(0); // 表示不等待屏幕刷新，直接更新图像

		m_Data.VSync = enabled; // 更新m_Data中的VSync成员变量
	}
